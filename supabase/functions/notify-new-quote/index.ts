...
// ============================================
//
// Called directly from the app after a quote is inserted.
// No database webhook needed — the frontend calls this via
// supabase.functions.invoke('notify-new-quote', { body: quoteData }).
//
...
//
// To deploy:
//   mkdir -p supabase/functions/notify-new-quote
//   cp supabase-edge-function.ts supabase/functions/notify-new-quote/index.ts
...
// ── Main handler ──────────────────────────────────────────
serve(async (req) => {
  // CORS preflight (needed when calling from the browser via supabase.functions.invoke)
  ...
  try {
    ...
    //   • Direct call from app: { contact_name, email, ... }
    //   • Webhook format:       { record: { contact_name, email, ... } }
    ...
    return new Response(
      ...
      { status: adminRes.ok ? 200 : 500, headers: corsHeaders }
    );
  } catch (err) {
